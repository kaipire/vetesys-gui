...
from controlador.vcontrol import BalanceadoControlador
from modulo.vmodel import Balanceado
...
class Listar(Frame):
    ...
    __controlador = BalanceadoControlador()

    def __init__(self, panel_master):
        super().__init__(master=panel_master)
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text=' .:: LISTA DE BALANCEADOS ::.', font=text_font1)
        ...
        lista_balanceados = self.get_controlador().get_lista_objetos()

        for lista in lista_balanceados:
...
class Registrar(Frame):
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self, text=" .:: REGISTRO DE BALANCAEDOS ::. ",  font=text_font1, pady = 10)
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self, text="Guardar", bg = 'white', command=self.guardar_balanceado) 
    ...
    def guardar_balanceado(self):
        ...
        balanceado = Balanceado(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)


        try:
            self.get_controlador().crear(balanceado) 
...
class Buscar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text='.:: BUSCAR BALANCEADO ::.', font=text_font1)
        self.nombre_opcion.pack()
        self.panel_buscar = None
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta2 = Label(master=self.panel1, text='Ingrese el codigo del Balanceado:', font=text_font1)
        self.etiqueta2.pack(side='left', pady=10)
        self.codigo = Entry(master=self.panel1, width = 30)
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        #boton buscar
        self.buscar_btn = Button(master=self.panel1, text='Buscar', font=text_font1, bg = 'white', command = self.buscar_balanceado)
    ...
    def buscar_balanceado(self):
        ''' Metodo que busca el objeto a traves del controlador y lo muestra en la pantalla.'''
        balanceado = self.get_controlador().buscar_cod(self.codigo.get())
        if not balanceado:
...
class Modificar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta = Label(master=self.panel1, text='Ingrese el codigo del Balanceado:', font=text_font1)
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self.panel2, text=" .:: MODIFICAR DATOS DE UN BALANCEADO::. ",  font=text_font1, pady = 10)
            self.__titulo_lbl.grid(row=0, column=0, columnspan=2)
        return self.__titulo_lbl
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self.panel2, text="Guardar", bg = 'white', command=self.guardar_balanceado) 
    ...
    def guardar_balanceado(self):
        ...
        balanceado = Balanceado(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)
        try:
            self.get_controlador().actualizar(balanceado) 
    ...
    def buscar_balanceado(self):
        ...
        if not balanceado:
            ...
            # insertamos
            self.get_codigo_entry().insert(0, balanceado.get_codigo())
            self.get_nombre_entry().insert(0, balanceado.get_nombre())
            self.get_marca_entry().insert(0, balanceado.get_marca())
            self.get_descripcion_entry().insert(0, balanceado.get_descripcion())
            self.get_fecha_caducidad_entry().insert(0, balanceado.get_fecha_de_caducidad())
            self.get_precio_entry().insert(0, balanceado.get_precio())
            self.get_existencia_entry().insert(0, balanceado.get_existencia())
...
class Borrar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.__panel_master = panel_master
        self.pack()
        self.nombre_borrar = tk.Label(master=self, text='.:: BORRAR BALANCEADO ::.', font=text_font1)
        ...
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        self.borrar_boton = Button(master=self.panel1, text='Borrar', font=text_font1, bg='white',command = self.borrar_balanceado)
    ...
    def borrar_balanceado(self):
        '''Metoro que busca el objeto a traves del controlador por ID y lo elimina. '''
        codigo = self.codigo.get()
        balanceado = self.get_controlador().buscar_cod(codigo)
        try:
            self.get_controlador().borrar(balanceado)
...
class Venta(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.nombre_borrar = tk.Label(master=self, text='.:: VENTA DE BALANCEADOS ::.', font=text_font1)
        ...
        self.vender_boton = Button(master=self.panel1, text='Vender', font=text_font1, bg='white',command = self.vender_balanceado)
        self.vender_boton.grid(row = 2, column = 1)
        #boton atras
        self.atras_boton = tk.Button(master = self.panel1, text='Volver', font=text_font1,bg='white',command=self.master.destroy)
        self.atras_boton.grid(row = 2, column = 2) 

    def vender_balanceado(self):
        balanceado = self.get_controlador().buscar_cod(self.codigo.get())
        cantidad = int(self.cantidad.get())
        if not balanceado:
            messagebox.showinfo("", "Registro no existe")
        elif int(balanceado.get_existencia()) <= 0:
            messagebox.showinfo("", "Balanceado agotado")
        elif cantidad > int(balanceado.get_existencia()):
            messagebox.showinfo("", "Supera el stock")
        else:
            costo = cantidad * int(balanceado.get_precio())
            messagebox.showinfo("Total a cobrar:  ", str(costo) + " gs")
            disminuir = int(balanceado.get_existencia()) - cantidad
            balanceado.set_existencia(disminuir)
            self.get_controlador().actualizar(balanceado)
