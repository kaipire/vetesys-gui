...
from controlador.vcontrol import MedicamentoControlador
from modulo.vmodel import Medicamento
...
class Listar(Frame):
    ...
    __controlador = MedicamentoControlador()

    def __init__(self, panel_master):
        super().__init__(master=panel_master)
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text=' .:: LISTA DE MEDICAMENTOS ::.', font=text_font1)
        ...
        lista_medicamentos = self.get_controlador().get_lista_objetos()

        for lista in lista_medicamentos:
...
class Registrar(Frame):
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self, text=" .:: REGISTRO DE MEDICAMENTOS ::. ",  font=text_font1, pady = 10)
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self, text="Guardar", bg = 'white', command=self.guardar_medicamento) 
    ...
    def guardar_medicamento(self):
        ...
        medicamento = Medicamento(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)


        try:
            self.get_controlador().crear(medicamento) 
...
class Buscar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text='.:: BUSCAR MEDICAMENTO ::.', font=text_font1)
        self.nombre_opcion.pack()
        self.panel_buscar = None
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta2 = Label(master=self.panel1, text='Ingrese el codigo del Medicamento:', font=text_font1)
        self.etiqueta2.pack(side='left', pady=10)
        self.codigo = Entry(master=self.panel1, width = 30)
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        #boton buscar
        self.buscar_btn = Button(master=self.panel1, text='Buscar', font=text_font1, bg = 'white', command = self.buscar_medicamento)
    ...
    def buscar_medicamento(self):
        ...
        medicamento = self.get_controlador().buscar_cod(self.codigo.get())
        if not medicamento:
...
class Modificar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta = Label(master=self.panel1, text='Ingrese el codigo del Medicamento:', font=text_font1)
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self.panel2, text=" .:: MODIFICAR DATOS DE UN MEDICAMENTO::. ",  font=text_font1, pady = 10)
            self.__titulo_lbl.grid(row=0, column=0, columnspan=2)
        return self.__titulo_lbl
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self.panel2, text="Guardar", bg = 'white', command=self.guardar_medicamento) 
    ...
    def guardar_medicamento(self):
        ...
        medicamento = Medicamento(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)
        try:
            self.get_controlador().actualizar(medicamento) 
    ...
    def buscar_medicamento(self):
        ...
        if not medicamento:
            ...
            # insertamos
            self.get_codigo_entry().insert(0, medicamento.get_codigo())
            self.get_nombre_entry().insert(0, medicamento.get_nombre())
            self.get_marca_entry().insert(0, medicamento.get_marca())
            self.get_descripcion_entry().insert(0, medicamento.get_descripcion())
            self.get_fecha_caducidad_entry().insert(0, medicamento.get_fecha_de_caducidad())
            self.get_precio_entry().insert(0, medicamento.get_precio())
            self.get_existencia_entry().insert(0, medicamento.get_existencia())
...
class Borrar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.__panel_master = panel_master
        self.pack()
        self.nombre_borrar = tk.Label(master=self, text='.:: BORRAR MEDICAMENTO ::.', font=text_font1)
        ...
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        self.borrar_boton = Button(master=self.panel1, text='Borrar', font=text_font1, bg='white',command = self.borrar_medicamento)
    ...
    def borrar_medicamento(self):
        '''Metoro que busca el objeto a traves del controlador por ID y lo elimina. '''
        codigo = self.codigo.get()
        medicamento = self.get_controlador().buscar_cod(codigo)
        try:
            self.get_controlador().borrar(medicamento)
...
class Venta(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.nombre_borrar = tk.Label(master=self, text='.:: VENTA DE MEDICAMENTOS ::.', font=text_font1)
        ...
        self.vender_boton = Button(master=self.panel1, text='Vender', font=text_font1, bg='white',command = self.vender_medicamento)
    ...
    def vender_medicamento(self):
        medicamentos = self.get_controlador().buscar_cod(self.codigo.get())
        cantidad = int(self.cantidad.get())
        if not medicamento:
            messagebox.showinfo("", "Registro no existe")
        elif int(medicamento.get_existencia()) <= 0:
            messagebox.showinfo("", "medicamento agotado")
        elif cantidad > int(medicamento.get_existencia()):
            messagebox.showinfo("", "Supera el stock")
        else:
            costo = cantidad * int(medicamento.get_precio())
            messagebox.showinfo("Total a cobrar:  ", str(costo) + " gs")
            disminuir = int(medicamento.get_existencia()) - cantidad
            medicamento.set_existencia(disminuir)
            self.get_controlador().actualizar(medicamento)
...
if __name__ == '__main__':
    root = tk.Tk()
    medicamento = AbmMedicamento(root)
    root.mainloop()
