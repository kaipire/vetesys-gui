...
from controlador.vcontrol import AccesorioControlador
from modulo.vmodel import Accesorio
...
class Listar(Frame):
    ...
    __controlador = AccesorioControlador()

    def __init__(self, panel_master):
        super().__init__(master=panel_master)
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text=' .:: LISTA DE ACCESORIOS ::.', font=text_font1)
        ...
        lista_accesorios = self.get_controlador().get_lista_objetos()
...
class Registrar(Frame):
    ...
    def inicializar(self):
        ...
        self.__panel_master.title(msg('abm.accesorio.titulo.registrar'))
        self.get_titulo_lbl()
        self.get_codigo_lbl()
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self, text=" .:: REGISTRO DE ACCESORIOS ::. ",  font=text_font1, pady = 10)
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self, text="Guardar", bg = 'white', command=self.guardar_accesorio) 
    ...
    def guardar_accesorio(self):
        ...
        accesorio = Accesorio(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)


        try:
            self.get_controlador().crear(accesorio) 
...
class Buscar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.pack(ipadx=50, ipady=50)
        self.__panel_master = panel_master
        self.nombre_opcion = Label(master=self, text='.:: BUSCAR ACCESORIO ::.', font=text_font1)
        self.nombre_opcion.pack()
        self.panel_buscar = None
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta2 = Label(master=self.panel1, text='Ingrese el codigo del Accesorio:', font=text_font1)
        self.etiqueta2.pack(side='left', pady=10)
        self.codigo = Entry(master=self.panel1, width = 30)
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        #boton buscar
        self.buscar_btn = Button(master=self.panel1, text='Buscar', font=text_font1, bg = 'white', command = self.buscar_accesorio)
    ...
    def buscar_accesorio(self):
        ...
        accesorio = self.get_controlador().buscar_cod(self.codigo.get())
        if not accesorio:
            ...
            Label(master=panel2,
                  text='Codigo : {}'.format(accesorio.get_codigo()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 1, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Nombre : {}'.format(accesorio.get_nombre()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 2, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Marca: {}'.format(accesorio.get_marca()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 3, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Descripcion: {}'.format(accesorio.get_descripcion()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 4, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Fecha de Caducidad: {}'.format(accesorio.get_fecha_de_caducidad()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 5, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Precio: {}'.format(accesorio.get_precio()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 6, column = 0, sticky = 'w')
            Label(master=panel2,
                  text='Existencia: {}'.format(accesorio.get_existencia()),
                  fg='purple', anchor='nw', font=text_font1).grid(row = 7, column = 0, sticky = 'w')
...
class Modificar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.panel1 = Frame(master=self)
        self.panel1.pack(ipady=10, fill='x')
        self.etiqueta = Label(master=self.panel1, text='Ingrese el codigo del Accesorio:', font=text_font1)
    ...
    def inicializar(self):
        ''' Metodo que hace la llamada a otros metodos para que se visualicen en pantalla'''
        self.__panel_master.geometry('800x600')
        self.__panel_master.title(msg('abm.accesorio.titulo.actualizar'))
        self.panel2 = Frame(master=self)
    ...
    def get_titulo_lbl(self):
        ...
        if not self.__titulo_lbl:
            self.__titulo_lbl = tk.Label(master=self.panel2, text=" .:: MODIFICAR DATOS DE UN ACCESORIO::. ",  font=text_font1, pady = 10)
            self.__titulo_lbl.grid(row=0, column=0, columnspan=2)
        return self.__titulo_lbl
    ...
    def get_guardar_btn(self):
        if not self.__guardar_btn:
            self.__guardar_btn = tk.Button(master=self.panel2, text="Guardar", bg = 'white', command=self.guardar_accesorio) 
    ...
    def guardar_accesorio(self):
        ...
        accesorio = Accesorio(codigo=codigo, nombre=nombre, marca=marca, descripcion=descripcion, fecha_de_caducidad=fecha_caducidad, precio=precio, existencia=existencia)
        try:
            self.get_controlador().actualizar(accesorio) 
    ...
    def buscar_accesorio(self):
        ...
        if not accesorio:
            ...
            # insertamos
            self.get_codigo_entry().insert(0, accesorio.get_codigo())
            self.get_nombre_entry().insert(0, accesorio.get_nombre())
            self.get_marca_entry().insert(0, accesorio.get_marca())
            self.get_descripcion_entry().insert(0, accesorio.get_descripcion())
            self.get_fecha_caducidad_entry().insert(0, accesorio.get_fecha_de_caducidad())
            self.get_precio_entry().insert(0, accesorio.get_precio())
            self.get_existencia_entry().insert(0, accesorio.get_existencia())
...
class Borrar(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.__panel_master = panel_master
        self.pack()
        self.nombre_borrar = tk.Label(master=self, text='.:: BORRAR ACCESORIO ::.', font=text_font1)
        ...
        self.codigo.pack(side='left')
        self.codigo.focus_force()
        self.borrar_boton = Button(master=self.panel1, text='Borrar', font=text_font1, bg='white',command = self.borrar_accesorio)
    ...
    def borrar_accesorio(self):
        '''Metoro que busca el objeto a traves del controlador por ID y lo elimina. '''
        codigo = self.codigo.get()
        accesorio = self.get_controlador().buscar_cod(codigo)
        try:
            self.get_controlador().borrar(accesorio)
...
class Venta(Frame):
    ...
    def __init__(self, panel_master):
        ...
        self.nombre_borrar = tk.Label(master=self, text='.:: VENTA DE ACCESORIOS ::.', font=text_font1)
        ...
        self.vender_boton = Button(master=self.panel1, text='Vender', font=text_font1, bg='white',command = self.vender_accesorio)
    ...
    def vender_accesorio(self):
        ...
        if not accesorio:
            messagebox.showinfo("", "Registro no existe")
        elif int(accesorio.get_existencia()) <= 0:
            messagebox.showinfo("", "Accesorio agotado")
        elif cantidad > int(accesorio.get_existencia()):
            messagebox.showinfo("", "Supera el stock")
        else:
            costo = cantidad * int(accesorio.get_precio())
            messagebox.showinfo("Total a cobrar:  ", str(costo) + " gs")
            disminuir = int(accesorio.get_existencia()) - cantidad
            accesorio.set_existencia(disminuir)
            self.get_controlador().actualizar(accesorio)
